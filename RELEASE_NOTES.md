...
- Windows 开始菜单中的卸载快捷方式改用 Windows 系统卸载图标，避免与软件启动或说明入口混淆。
- Linux 卸载流程补充用户 WPS 加载项注册清理：remove 时清理普通用户主目录下的加载项副本、publish.xml 和 jsplugins.xml 中的本项目条目；purge 时继续清理配置目录和运行数据。
- README 增加 Windows、银河麒麟系统、UOS系统的卸载方式和清理范围说明。
- README 精简安装说明和朗读能力描述，不再展开固定词汇的具体读法。
...
| 目标 | 文件 |
| --- | --- |
| x86/x64 Windows 10/11 | dist/wps-read-aloud-comate_1.2.0_windows.exe |
| x64 银河麒麟系统 | dist/wps-read-aloud-comate_1.2.0_amd64.deb |
| ARM64 银河麒麟系统 | dist/wps-read-aloud-comate_1.2.0_arm64.deb |
| x64 UOS系统 | dist/cn.wps-read-aloud-comate_1.2.0_amd64.deb |
| ARM64 UOS系统 | dist/cn.wps-read-aloud-comate_1.2.0_arm64.deb |
