...
- 增加“当前位置朗读”，并将按钮顺序调整为“全文朗读、当前位置朗读、选区朗读”。
...
- 按钮名称调整为“全文朗读、当前位置朗读、选区朗读、暂停朗读、继续朗读、停止朗读、朗读语速、朗读音量、状态检查、关于朗读”。
- 语速和音量改为大号菜单按钮，和其他 Ribbon 按钮保持同一视觉高度。
- 图标资源扩大到 128×128 透明画布，并新增朗读语速、朗读音量图标。
- Piper 合成参数改为更稳定的噪声设置，减少音调和语速漂移。
- 当前内置 `zh_CN.onnx` 为 `x_low` 质量模型；如需显著提升普通话自然度，建议替换为更高质量普通话 Piper 模型。
- 前端增加按钮连点节流和播放状态保护。
