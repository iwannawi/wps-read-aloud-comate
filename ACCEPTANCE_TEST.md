...
| --- | --- | --- |
| x86/x64 Windows 10/11 | WPS Office 2019 或更高版本 | wps-read-aloud-comate_1.1.17_windows.exe |
| x64 银河麒麟 V10 及以上 | WPS Office 2019 for Linux 或更高版本 | wps-read-aloud-comate_1.1.17_amd64.deb |
| ARM64 银河麒麟 V10 及以上 | WPS Office 2019 for Linux 或更高版本 | wps-read-aloud-comate_1.1.17_arm64.deb |
| x64 UOS V20 | WPS Office 2019 for Linux 或更高版本 | cn.wps-read-aloud-comate_1.1.17_amd64.deb |
| ARM64 UOS V20 | WPS Office 2019 for Linux 或更高版本 | cn.wps-read-aloud-comate_1.1.17_arm64.deb |

## 安装验收

| CPU 架构 + 操作系统 | 操作 |
| --- | --- |
| x86/x64 Windows 10/11 | 运行 wps-read-aloud-comate_1.1.17_windows.exe |
| x64 银河麒麟 V10 及以上 | sudo dpkg -i wps-read-aloud-comate_1.1.17_amd64.deb |
| ARM64 银河麒麟 V10 及以上 | sudo dpkg -i wps-read-aloud-comate_1.1.17_arm64.deb |
| x64 UOS V20 | sudo dpkg -i cn.wps-read-aloud-comate_1.1.17_amd64.deb |
| ARM64 UOS V20 | sudo dpkg -i cn.wps-read-aloud-comate_1.1.17_arm64.deb |
...
- Windows 不写入当前用户 Run 自启动项，不创建常驻后台服务。
- Windows 安装器会请求管理员权限，用于修改 WPS 安装目录 office6/cfgs/oem.ini。
- Windows 安装完成后提示重启电脑；重启后 WPS 应能读取 OEM 配置并显示“文档朗读”选项卡。
- Windows 开始菜单存在卸载入口，系统“应用和功能”中存在本软件卸载项。
- Windows 当前用户 WPS jsaddons 目录下应存在“文档朗读助手_版本号”目录；安装目录下应存在同名 .7z 离线包，文件头应为 7z 格式。
...
| 服务管理 | OEM + publish 离线模式加载选项卡；语音合成子进程只在朗读时运行；不写入 Run 自启动 | systemd 管理 wps-read-aloud-comate.service | systemd 管理 wps-read-aloud-comate.service |
...
示例文本：
    ...
    这是 WPS 2026 read aloud test，版本是 v1.1.17。
