...
朗读时会按完整语句切分、逐句合成并播放；加载项会在 WPS 文档中选中当前朗读语句，进入下一句时同步选中下一句。顶部选项卡提供 7 个入口：开始朗读、停止朗读、连页朗读、当页朗读、朗读语速、状态检查、关于朗读。默认使用“连页朗读”，从当前光标处读到文档末尾；切换到“当页朗读”后，从当前光标处读到当前页末尾。服务端会先预生成前三句音频，再开始播放第一句，减少句间等待。
